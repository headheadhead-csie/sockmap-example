...
int main(int argc, char *argv[]) {
    ...
    int tcp_server_port, tcp_fd, tcp_lserver_fd;
    in_addr_t tcp_s_addr;
    int vsk_server_port, vsk_server_fd;
    struct sockaddr_in tcp_server_addr, tcp_local_addr, tcp_lserver_addr;
    ...
    struct sock_key_pair tcp_lserver_skp;
    ...
    tcp_lserver_fd = set_tcp_server(&tcp_lserver_addr, vsk_server_port);
    tcp_lserver_skp.sock_fd = tcp_lserver_fd;
    tcp_lserver_skp.stub_fd = -1;
    event.events = EPOLLIN;
    event.data.ptr = &tcp_lserver_skp;
    add_event(epoll_fd, tcp_lserver_fd, &event);
    ...
    while (true) {
        ...
        int new_conn, stub_conn, flag, event_cnt;
        ...
        for (int i = 0; i < event_cnt; i++) {
            ...
            if (skp->sock_fd == tcp_lserver_fd) {
                if ((stub_conn = accept4(tcp_lserver_fd, NULL, NULL, 0)) < 0) {
                    perror("accept guest fail");
                    exit(errno);
                }
